...
@section('content')
    ...
    <x-form.auth-forms title="Вход в аккаунт" method="POST" action="{{ route('login.handle') }}">
        ...
        @enderror
        <x-form.text-input type="password" placeholder="Пароль" name="password" required is-error="{{ $errors->has('password') || $errors->has('email') }}"/>
        @error('password')
        <x-form.error>
            {{ $message }}
        </x-form.error>
