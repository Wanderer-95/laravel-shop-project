...
@section('content')
    ...
    <x-form.auth-forms title="Регистрация" method="POST" action="{{ route('register.handle') }}">
        <x-form.text-input type="text" placeholder="Введите имя" name="name" required/>
        <x-form.text-input type="email" placeholder="E-mail" name="email" required
        ...
        <x-slot:socialAuth>
            <ul class="space-y-3 my-2">
                <li>
                    <a href="{{ route('redirect', ['driver' => 'github']) }}" class="relative flex items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs">
                    Есть аккаунт?
                    <a href="{{ route('login') }}" class="text-white hover:text-white/70 font-bold underline underline-offset-4">
                        Войти
                    </a>
                </div>
            </div>
